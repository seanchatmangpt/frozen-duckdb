...
| `docs/sjira/v26.9.21/C[0-9]*.md` | `scripts/gen_sjira_tickets_wave4.sh` | pending | Renderer + outputs land with the wave-4 merge (main-checkout commit 400fba1); this branch was cut at 304a958, before that commit. Flip to active once the renderer is in the tree. | wave-4 marketplace capability |
...
| `Makefile` | `hand:` | active | SOURCE: sync / genesis-check / gates targets (C10). `verify` is deliberately NOT declared here — ticket C8 owns Makefile verify; later merge wins and reconciles. | C10 |
