...
| `docs/sjira/v26.9.21/T[0-9]*.md` | `scripts/gen_sjira_tickets.sh` | active | Edit the SPECS table in the script, re-run it; rendered tickets are consequences. Ticket History sections are the sanctioned append-only exception (票 law). Rendered frontmatter carries `aps:standing` from the script's per-id standing map (terminal History standings; new tickets default OPEN), checked by `scripts/check_sjira_standing.sh`. | wave-2 ticket wave |
...
| `docs/sjira/v26.9.21/C[0-9]*.md` | `scripts/gen_sjira_tickets_wave4.sh` | active | Renderer + outputs landed with the wave-4 merge (commit 400fba1); pending row flipped to active by the wave-4 dry run (2026-09-21) — genesis-check correctly refused sync/gates until this flip. Edit the SPECS table in the script, re-run it; ticket History sections are the sanctioned append-only exception (票 law). Standing column as in the T row. | wave-4 marketplace capability |
...
| `scripts/verify-evidence.ttl` | `scripts/verify-gates.sh` | active | Runtime evidence record (記): re-written by every verify-gates run with observed exit codes; each run also records `ver:subjectHead` / `ver:subjectTree` (`git rev-parse HEAD` / `git rev-parse HEAD^{tree}`), pinning the evidence chain to the exact verified subject — a committed verify-evidence.ttl proves which HEAD/tree passed the 5/5 gate battery (commit b9fa77c); committed as 証 (pattern: commit f3120fd). | C8 |
...
| `scripts/check_sjira_standing.sh` | `hand:` | active | Standing gate (FDDB-26922-03): verifies each v26.9.21 ticket's frontmatter `aps:standing` equals its final History standing, so the frontmatter cannot drift from the History record. First run: checked=45 mismatches=0 (commit ed27449). | FDDB-26922-03 |
