...
| crates/frozen-duckdb/src/cli/commands.rs, crates/frozen-duckdb/tests/cli_surface_tests.rs | clap argument-surface repair: Convert `--input-format`/`--output-format` and Summarize `--max-length` demoted to long-only (duplicate shorts `-i`/`-m` made clap debug asserts panic at parse, exit 101 — `convert`/`summarize` could not run at all; witnessed by G3 via `frozen-duckdb-cli convert --help`), plus a `debug_assert()` tripwire test rendering every subcommand's help | no pack/gate expresses CLI argument-surface uniqueness or help-render verification | frozen-duckdb-cli pack — argument-surface family (unadmitted) | 2026-09-21 (G3) |
